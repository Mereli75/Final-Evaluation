...
app = dash.Dash("Automobile Statistics Dashboard")
...
@app.callback(
    ...
    [Input(component_id='select-year', component_property='disabled'), Input(component_id='dropdown-statistics', component_property='value')])
...
# Plot 3 Pie chart for total expenditure share by vehicle type during recessions
        ...
        R_chart3 = dcc.Graph(
                 ...
                 title="Total Expenditure Share by Vehicle during Recessions"))

# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        un_rec=recession_data.groupby('Unemployment_Rate','Vehicle_Type')['Automobile_Sales'].sum().reset_index()
        R_chart4 = dcc.Graph(figure=px.bar(
            x='Unemployment_Rate',
            y='Automobile_Sales',
            title="Automobile Sales per Type of Vehicle and Unemployment Rate"))
...
# Plot 2 Total Monthly Automobile sales using line chart.
        ...
        Y_chart3 = dcc.Graph(figure=px.bar(
            x='Unemployment_Rate',
            y='Automobile_Sales',
            title='Average Vehicles Sold by Vehicle Type in the year {2023}'.format(input_year)))

            # Total Advertisement Expenditure for each vehicle using pie chart
        exp_data=yearly_data.groupby('Advertising_Expenditure')['Automobile_Sales'].mean().reset_index()
        Y_chart4 = dcc.Graph(figure=px.pie(
           values='Advertising_Expenditure',
                 names='Automobile_Sales',
                 title="Total Expenditure Share by Vehicle"))
